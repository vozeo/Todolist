...
public class ClockView extends View {  //定义View的子类，用于显示番茄钟界面
    ...
    private int centerY;
    private int radius;
    private final Paint mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);  //ANTI_ALIAS_FLAG:抗锯齿
    ...
    private String formatTime(int time) {  //把时间（分钟）转换成字符串
        ...
        if (time < 10) {
            sb.append("0").append(time).append(":00");
        } else {
            sb.append(time).append(":00");
        }
        return sb.toString();
    }
